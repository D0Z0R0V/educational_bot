...
from .BasicPath import BasicPath

from memoria import make_hash_sha256
from send2trash import send2trash

class Path(BasicPath):
	...
	def __hash__(self):
		return make_hash_sha256(self.absolute_path, base=32)
	...
	def __add__(self, other):
		if isinstance(other, str):
			other_string = other
		elif isinstance(other, BasicPath):
			other_string = other.string
		else:
			raise TypeError(f'{other} is a {type(other)} but it should either be string or {BasicPath}')
	...
	def delete(self, name=None):
		...
		if to_delete.exists():
			send2trash(to_delete.path)
		else:
			raise FileNotFoundError(f'Path: "{to_delete.path}" not found!')

		# check to see if it is actually deleted
		if to_delete.exists():
			raise FileExistsError(f'Path: failed to delete "{to_delete.path}"')

	def delete_directory(self, name):
		delete_dir(path=(self+name).path)

	def pickle(self, obj, method='pickle', mode='wb'):
		_pickle(path=self.path, obj=obj, method=method, mode=mode)

	def unpickle(self, method='pickle', mode='rb'):
		return _unpickle(path=self.path, method=method, mode=mode)
...
class Directory(BasicPath):
	def __init__(self, string, show_size=False):
		super().__init__(string=string, show_size=show_size)
		if self.exists():
			if not self.is_directory():
				raise FileExistsError(f'{self.string} is a file!')
...
class File(BasicPath):
	def __init__(self, string, show_size=False):
		...
		if self.exists():
			if not self.is_file():
				raise IsADirectoryError(f'{self.string} is a directory!')
